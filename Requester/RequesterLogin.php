...
if(!isset($_SESSION['is_login'])){
  if(isset($_REQUEST['rEmail'])){
    ...
    $sql = "SELECT r_email, r_password FROM requesterlogin_tb WHERE r_email='".$rEmail."' AND r_password='".md5($rPassword)."' limit 1";
    $sqla = "SELECT r_email, r_password FROM experthead WHERE r_email='".$rEmail."' AND r_password='".md5($rPassword)."' limit 1";
    $sqlb = "SELECT r_email, r_password FROM experts WHERE r_email='".$rEmail."' AND r_password='".md5($rPassword)."' limit 1";
    $sqlc = "SELECT r_email, r_password FROM storemanager WHERE r_email='".$rEmail."' AND r_password='".md5($rPassword)."' limit 1";
    ...
    if($resulta->num_rows == 1){
      ...
      echo "<script> location.href='ExpertheadProfile.php'; </script>";
      exit;
    } else {
      $msg = '<div class="alert alert-warning mt-2" role="alert"> Enter Valid Email and Password </div>';
    }
