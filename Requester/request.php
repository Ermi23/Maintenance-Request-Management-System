...
<div class="col-sm-4 mb-5">
  ...
  <?php 
 $sql = "SELECT request_id, request_info, request_desc, request_date, Approval FROM submitrequest_tb WHERE Approval='Approved' && Assigned='Not_Assigned'";
 ...
 if($result->num_rows > 0){
  while($row = $result->fetch_assoc()){
   ...
   echo '<p class="card-text">Approval: ' . $row['Approval'] . '</p>';
   echo '<div class="float-right">';
   echo '<form action="" method="POST"> <input type="hidden" name="id" value='. $row["request_id"] .'><input type="submit" class="btn btn-danger mr-3" name="view" value="View"></form>';
   echo '</div>' ;
   echo '</div>' ;
   echo'</div>';
  }
  ...
  <p>All Requests assigned successfully  .</p>
  <hr>
  <h5 class="mb-0">No Approved Requests Yet</h5>
</div>';
 }
