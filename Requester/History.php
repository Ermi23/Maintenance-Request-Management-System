...
<div class="col-sm-9 col-md-10 mt-5">
  <?php 
 $sql = "SELECT * FROM submitrequest_tb WHERE requester_email='$aEmail' AND trash='unseen'";
 ...
 if($result->num_rows > 0){
  ...
  while($row = $result->fetch_assoc()){
    ...
    <td>'.$row["Approval"].'</td>
    <td>'.$row["Assigned"].'</td>
    <td><form action="" method="POST" class="d-inline"><input type="hidden" name="id" value='. $row["request_id"] .'><button type="submit" class="btn btn-danger" name="remove" value="remove">Remove</button></form></td>
